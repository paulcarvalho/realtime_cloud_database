...
-- WHILE (@RC > 0)
BEGIN
    -- Get the next 20 tag codes for running the filter
    SELECT TOP (5) @NextBatchMax = tag_key
    FROM tmp_tags
    WHERE tag_key > @LargestKeyProcessed
        AND tag_key <= 1000 -- Use this line to test specific number of tag codes
    ORDER BY tag_key ASC;

    -- Save the next 20 tag codes in a temporary table
    SELECT TagCode
    INTO #tmp_table1
    FROM tmp_tags
    WHERE tag_key > @LargestKeyProcessed
        AND tag_key <= @NextBatchMax;

    -- IMPORTANT NOTE: set @RC needs to immediately follow the above lines because it tracks the number of rows accessed in that process
    SET @RC = @@ROWCOUNT;

    -- Find the number of detections rows for the current tags
    SELECT @tagCount = COUNT(*) 
        ...
        WHERE EXISTS(
            SELECT 1
            FROM #tmp_table1
            WHERE #tmp_table1.TagCode = detects.TagCode
        )
    ...
    IF @tagCount > 20000
        ...
        PRINT @tagCount;
        -- Get rows in detects for tag codes in the temporary table and merge this with receiver information
        WITH cte_detects1 (recv_ID, location, recv, DateTime_Orig, TagCode, DateTime_PST, Temp, filename, general_location, latitude, longitude, rkm)
        AS
        (
        ...
        cte_detects2 (TagCode, general_location, n_detects)
        AS
        (
            SELECT TagCode, general_location, n_detects = COUNT(*)
            FROM cte_detects1
            GROUP BY TagCode, general_location
        ),

        cte_detects3 (TagCode, general_location, n_detects)
        AS
        (
            SELECT TagCode, general_location, n_detects
            FROM cte_detects2
            WHERE n_detects >= 20000
        )
        
        SELECT * FROM cte_detects3;
...
SELECT TagCode, DateTime_PST, general_location
FROM detects_with_locations
WHERE TagCode = 'F0D2' AND
    general_location = 'ButteBrRT';
